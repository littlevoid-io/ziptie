...
Describe "Ziptie Lockdown Script Verification" {
    ...
    AfterAll {
        ...
        $restoredUtilsDir = Resolve-Path "$PSScriptRoot/../scripts/utils" -ErrorAction SilentlyContinue
        
        if ($restoredUtilsDir -and (Test-Path $utilsBackupPath)) {
            $backupFiles = Get-ChildItem -Path $utilsBackupPath -Filter "*.ps1"
            foreach ($bf in $backupFiles) {
                Copy-Item -Path $bf.FullName -Destination $restoredUtilsDir -Force
            }
