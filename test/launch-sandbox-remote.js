...
console.log(
  `\x1b[36m[Sandbox Config]\x1b[0m Targeting remote GitHub branch: \x1b[32m${branchName}\x1b[0m`
);
...
try {
  ...
  console.log(
    `\x1b[32mSuccessfully generated dynamic remote Sandbox WSB file at:\x1b[0m ${wsbPath}`
  );
  ...
  exec(`start "" "${wsbPath}"`, error => {
    ...
    console.log(
      `\x1b[32mClean Windows Sandbox spawned successfully! The one-line installer will execute inside the guest context directly fetching from the remote '${branchName}' branch.\x1b[0m`
    );
  });
} catch (err) {
